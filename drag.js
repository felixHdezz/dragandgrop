...
//Muestra el icono para modo edicion de la pagina 
$("#header > nav > div.header-link.dragdrop-show-btn").removeClass("hidden");

var DragAndDrop = DragAndDrop || {

    init: function () {
        //Function que inicializa el control de sortable 
    ...
    showhide: function (event) {
        ...
        var _dashboardpanel = $(this).closest(panel);
    ...
    addRow: function (event) {
        ...
        var _dashpanel = $(this).closest(panel);
    ...
    agregaColumns: function (rinditr, _rDash, _pDash) {
        for (var _col = 0; _col < _rDash.length; _col++) {
            if ($(_rDash[_col]).find(panel).attr('id') === _pDash.attr('id')) {
                ...
                if (_col == 0) {
                    if (nxt.length == 0 && $(_rDash[1]).attr('data-visible') == 'true') {
                    ...
                    if ((af.length != 0 && nxt.length != 0) && (af.length != 0 & lst.length == 0)) {
                        if (($(_rDash[_col]).attr('data-value-col') > 8) || ($(_rDash[_col]).attr('data-value-col') > 4 && $(_rDash[1]).attr('data-value-col') > 4)) {
                            $(_rDash[1]).css('display', 'none').attr('data-visible', 'false');
                    ...
                    if (lst.length == 0 && $(_rDash[2]).attr('data-visible') == 'true') {
                        $(_rDash[2]).css('display', 'none').attr('data-visible', 'false');
                    }else if (af.length == 0) {
                    ...
                    if (af.length != 0 && lst.length == 0) {
                        if ($(_rDash[_col]).attr('data-value-col') == 8 && $(_rDash[0]).attr('data-value-col') < 8) {
                            $(_rDash[2]).css('display', 'none').attr('data-visible', 'false');
                        } else {
                            $(_rDash[2]).css('display', 'none').attr('data-visible', 'false');
                    ...
                    if (af.length == 0 && (nxt.length != 0 && lst.length != 0)) {
                        if ($(_rDash[_col]).attr('data-value-col') > 8) {
                            $(_rDash[2]).css('display', 'none').attr('data-visible', 'false');
                            DragAndDrop.avanzaSiguienterow($(_rDash[2]).closest('div._rowindicador').next(), $(_rDash[2]).find(panel), $(_rDash[2]).attr('data-value-col'))
                        }
                    ...
                    if (af.length == 0 && $(_rDash[0]).attr('data-visible') == 'true') {
                        $(_rDash[0]).css('display', 'none').attr('data-visible', 'false');
                    ...
                    if ((nxt.length != 0 && lst.length != 0) && (af.length == 0 && lst.length != 0)) {
                        $(_rDash[0]).css('display', 'block').attr('data-visible', 'true');
                        DragAndDrop.avanzaSiguienterow($(_rDash[_col]).closest('div._rowindicador').next(), $(_rDash[_col]).find(panel), $(_rDash[_col]).attr('data-value-col'))
                    }
                    if ((nxt.length == 0 && lst.length != 0) && (af.length != 0 && lst.length != 0)) {
                        if ($(_rDash[_col]).attr('data-value-col') > 8) {
            ...
            for (var _col = _dcols.length - 1; _col >= 0; _col--) {
                ...
                if (val != 12) {
                    if ($(_dcols[_col]).find(panel).length == 0) {
                        ...
                        // Si exise un panel dentro de la zona de peligro
    ...
    ocultacolumnas: function (_rowsdashboard) {
        var _dashcolumns = $(_rowsdashboard).find(dashElement);
        if (_dashcolumns.length > 0) {
            for (var _column = 0; _column < _dashcolumns.length; _column++) {
                var _isdashpanel = $(_dashcolumns[_column]).find(panel);
                if (_isdashpanel.length == 0 && $(_dashcolumns[_column]).attr('data-visible') == 'true') {
                    $(_dashcolumns[_column]).css('display', 'none').attr('data-visible', 'false');
                    return false;
                }
            }
        }
        ...
        $(dashElement).sortable({
            ...
            update: function (event, ui) {
                ...
                if (_divId === "ColumnSeguridadEventos") {
                    $(_template).find(panel).css('cssText', 'margin-bottom: 0px !important;');
                } else {
                    $(_template).find(panel).css('cssText', 'margin-bottom: none');
                }

                // Verifica si el icono de showhidden esta oculto
                var _dashColumn = $(_template).find(panel).find('[class=panel-tools]');
                var _hpanel = $(_template).find(panel).find('div.hpanel');
    ...
    actualizaColumnas: function (_dashrow) {
        debugger;
        var countelement = $(_dashrow).find(dashElement).find(panel);
