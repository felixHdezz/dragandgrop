$(function(){
    ...
    var connect = "[name=dashboard-panel]";
    ...
    $(".dashboard-config").on("click", function(event) {
        ...
        if (icon.hasClass("pe-7s-config")) {
            $(this).parent().attr('title', 'Guardar Configuración');            
            ...
            $("div[name=dashboard-column]").each(function( columnIndex, columnElement) {
                $("div[id=" + $(columnElement).attr("id") + "] > [name=dashboard-panel]" ).each(function(panelIndex, panelElement) {
                    $(panelElement).css("display", "");
                });
            });
        } else {
            $(this).parent().attr('title', 'Configuración');
        ...
        $("div[name=dashboard-column]").each(function(columnIndex, columnElement) {
            $( "div[id=" + $(columnElement).attr("id") + "] > [name=dashboard-panel]").each(function(panelIndex, panelElement) {
                var panel = {
                    ...
                    title: $(panelElement).find("div.dashboard-panel-title:first")[0] .textContent, isVisible:
                    $(panelElement).attr("data-panel-visible") != undefined ? $(panelElement).attr("data-panel-visible") : true
                };
    ...
    function loadDashboardConfig() {
        ...
        if (currentConfig != undefined && currentConfig != null) {
            ...
            dataConfig.forEach(function(panel) {
                ...
                $("#" + panel.id).find( "div.dashboard-panel-title:first" )[0].textContent = panel.title;
